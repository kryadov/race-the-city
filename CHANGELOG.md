...
- The opening screen now shows a **built-in demo city — central Paris (Île de la Cité, the Seine
  ...
  (`public/demo/paris.json`, generated by `scripts/bake-demo-city.mjs`). It loads **instantly with no
  network** on the critical first paint, so a slow or refusing map server can no longer leave the very
  first screen blank — the bug where a cold start "sometimes doesn't load a location".
- **Boot no longer geocodes a random city.** A `?city=` share link still loads its real city, and
  **Continue** still resumes your saved session; otherwise you land in the demo and pick a real city
  from the menu whenever you want one. As a safety net, if loading a real city fails outright and
  nothing has loaded yet, the game falls back to the demo instead of a dead error screen.
- The demo flows through the exact same parse → build pipeline as a live city (roads, buildings, the
  Seine, baked terrain heights), so it looks and drives like any other — it's a real place, not a stub.
- 👀 Open the app with the network throttled or offline: the Paris demo should appear right away and
  be drivable, with the Seine, its islands and bridges all there.
